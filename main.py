...
def train(g_a, g_b, d_a, d_b, data_loader, g_optimizer, da_optimizer, db_optimizer):
    ...
    for real_a, real_b, _, _ in train_bar:
        ...
        g_optimizer.step()
        total_g_loss += generators_loss.item() * real_a.size(0)
        ...
        da_optimizer.step()
        total_da_loss += adversarial_loss.item() * real_a.size(0)
        ...
        adversarial_loss.backward()
        db_optimizer.step()
        total_db_loss += adversarial_loss.item() * real_a.size(0)

        total_num += real_a.size(0)
